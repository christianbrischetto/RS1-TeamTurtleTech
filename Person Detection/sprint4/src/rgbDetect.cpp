    ...
    cv::Mat detectBlue()
    {
        ...
        if (!rgb_image_.empty() && !depth_image_.empty())
        {
            ...
            if (!contours.empty()) {
                ...
                RCLCPP_INFO(this->get_logger(), "Blue person detected at angle %.2f degrees with raw depth value: %d meters. Position in robot frame: x=%.2f m, y=%.2f m.",
    ...
    cv::Mat detectRed()
    {
        ...
        if (!rgb_image_.empty() && !depth_image_.empty())
        {
            ...
            if (!contours.empty()) {
                ...
                // Print the angle and raw depth value
                RCLCPP_INFO(this->get_logger(), "Red person detected at angle %.2f degrees with raw depth value: %d meters. Position in robot frame: x=%.2f m, y=%.2f m.",
                angle, raw_depth_value, x, y);
            }
        }
